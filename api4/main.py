...
@app.route('/apia/csv', methods=['POST'])
def recibir_csv():
    global df
    global midf
    global usuariosp
    if request.method == 'POST':
        ...
        sep2 = rae.query('userId in @user_part_4')
        ...
        redis_conn.set('lsrae4', json.dumps(diccionario2))
        

        
        return jsonify({"mensaje": "csv cargado correctamente a redis 4"})
    else:
...
def recibir_datos():
    ...
    if request.method == 'POST':
        ...
        lsrae_cached = redis_conn.get('lsrae4')
